...
export default {
  ...
  methods: {
    async updatePrivacy() {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/updateprivacy`, {
          method: 'POST',
          credentials: 'include',
          body: JSON.stringify({
    ...
    async fetchInvitations() {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/getinvitationsfollow`, {
          method: 'GET',
          credentials: 'include'
        });
    ...
    async acceptInvitation(invitationId) {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/accepteinvi`, {
          method: 'POST',
          credentials: 'include',
          body: JSON.stringify({
    ...
    async rejectInvitation(invitationId) {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/followers?action=rejectInvitation&profileUser=${invitationId}`, {
          method: 'POST',
          credentials: 'include'
        });
        if (res.ok) {
    ...
    async checkprivacy() {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/checkmyprivacy`, {
        credentials: 'include',
      })
      const data = await res.json()
