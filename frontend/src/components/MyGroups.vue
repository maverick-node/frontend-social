...
export default {
  ...
  methods: {
    ...
    async fetchMyGroups() {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/mygroups`, {
          credentials: 'include'
        });
        if (response.ok) {
    ...
    async fetchPendingInvitations(groupId) {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/GetInvitations?group_id=${groupId}`, {
          credentials: 'include'
        });
        if (response.ok) {
    ...
    async leaveGroup(groupId) {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/removememberfromgroup`, {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
