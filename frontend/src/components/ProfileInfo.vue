        ...
        <div v-if="Array.isArray(posts) && posts.length > 0">
          <div v-for="post in posts" :key="post.id" class="post">
            ...
            <div class="post-content">
              ...
              <img
                v-if="post.image"
                :src="'http://social-net.duckdns.org:8080/uploads/' + post.image"
                alt="Post Image"
                class="post-image clickable-image"
                @click="showFullImage('http://social-net.duckdns.org:8080/uploads/' + post.image)"
                style="cursor:pointer"
              />
            </div>
            ...
            <div v-if="visibleCommentsPostId === post.id" class="comments-section">
              <ul>
                <li v-for="comment in Comments" :key="comment.id" class="comment-item">
                  ...
                  <img
                    v-if="comment.image"
                    :src="'http://social-net.duckdns.org:8080/uploads/' + comment.image"
                    alt="Comment Image"
                    class="comment-image clickable-image"
                    @click="showFullImage('http://social-net.duckdns.org:8080/uploads/' + comment.image)"
                    style="cursor:pointer"
                  />
                </li>
              </ul>
...
export default {
  ...
  methods: {
    ...
    getComments(postId) {
      return fetch(`http://social-net.duckdns.org:8080/api/getcomments?post_id=${postId}`, {
        method: 'GET',
        credentials: 'include'
      })
        .then(response => {
        ...
        .then(comments => {
          ...
          for (let i = 0; i < comments.length; i++) {
            if (!comments[i].Avatar) {
              ...
              comments[i].Avatar = 'http://social-net.duckdns.org:8080/uploads/' + comments[i].Avatar;
            }
          }
    ...
    checkIfFollowing() {
      ...
      fetch(`http://social-net.duckdns.org:8080/api/followers?action=isFollowing&profileUser=${this.user.username}`, {
        method: "GET",
        credentials: "include"
      })
        .then(response => {
    ...
    async fetchuserData(id) {
      try {
        const response = await fetch(`http://social-net.duckdns.org:8080/api/userinfo?user_id=${id}`, {
          method: "GET",
          credentials: "include"
        });
        ...
        if (!this.user.avatar) {
          ...
          this.user.avatar = 'http://social-net.duckdns.org:8080/uploads/' + this.user.avatar;
        }
    ...
    fetchUserPosts(username) {
      fetch(`http://social-net.duckdns.org:8080/api/ownposts?username=${username}`, {
        method: "GET",
        credentials: "include"
      })
        .then(response => {
    ...
    async follow(profileUser) {
      try {
        const response = await fetch(`http://social-net.duckdns.org:8080/api/followers?action=follow&profileUser=${profileUser}`, {
          method: 'POST',
          credentials: 'include'
        });
    ...
    async unfollow(profileUser) {
      try {
        const response = await fetch(`http://social-net.duckdns.org:8080/api/followers?action=unfollow&profileUser=${profileUser}`, {
          method: 'POST',
          credentials: 'include'
        });
